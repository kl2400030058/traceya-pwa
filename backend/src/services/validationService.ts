...
export async function validateGeofence(
  ...
  try {
    ...
    const rule = GEOFENCE_RULES.find(
 ...
 */
export async function validateSeason(
  species: string,
  timestamp: Date
): Promise<boolean> {
  try {
    // Find seasonal rules for the species
    const rule = SEASONAL_RULES.find(
      r => r.species.toLowerCase() === species.toLowerCase()
    );
 ...
 */
export function getAllowedRegions(species: string) {
  const rule = GEOFENCE_RULES.find(
    r => r.species.toLowerCase() === species.toLowerCase()
  );
 ...
 */
export function getAllowedMonths(species: string): number[] {
  const rule = SEASONAL_RULES.find(
    r => r.species.toLowerCase() === species.toLowerCase()
  );
 ...
 */
export function calculateQualityScore(event: {
 ...
 */
export async function validateCollectionEvent(event: {
  species: string;
  latitude: number;
  longitude: number;
  timestamp: Date;
  ...
  const isValidLocation = await validateGeofence(
    event.latitude,
    event.longitude,
    event.species
  );

  const isValidSeason = await validateSeason(
    event.species,
    event.timestamp
  );
